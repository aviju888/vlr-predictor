...
async def load_vct_teams():
    ...
    vct_file = project_root / "backend" / "data" / "vct_2025_teams.json"
...
async def collect_vct_data():
    ...
    try:
        ...
        if len(vct_matches) == 0:
            ...
            output_file = project_root / "backend" / "data" / "vlr_matches_180d.csv"
            matches_df.to_csv(output_file, index=False)
            print(f"💾 Saved all matches to: {output_file}")
            
        else:
            # Save VCT matches
            vct_output = project_root / "backend" / "data" / "vct_2025_matches.csv"
        ...
        main_output = project_root / "backend" / "data" / "map_matches_365d.csv"
        ...
        mapping_file = project_root / "backend" / "data" / "vct_team_mapping.json"
